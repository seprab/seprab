...
- **Blog**: content collection defined in `src/content.config.ts`; one `.md` in `src/content/blog/` per post (`title`, `description`, `pubDate`, `tags`, optional `draft`). Index, RSS (`/rss.xml`), and sitemap are generated — there is no manual post index. **`draft: true` posts render in `npm run dev` but are completely excluded from production builds**; approving a post = removing the flag.
